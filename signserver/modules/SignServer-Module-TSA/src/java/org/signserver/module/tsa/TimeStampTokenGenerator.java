/* This file has been backported from BC v1.56, in order to add support for ESSCertIDv2. (pruiz) */
...
public class TimeStampTokenGenerator
{
        ...
        throws IllegalArgumentException, TSPException
    {
        ...
        try
        {
            ...
            {
		final ASN1Integer serial = new ASN1Integer(assocCert.getSerialNumber());
                final ESSCertID essCertid = new ESSCertID(digestCalculator.getDigest(),
                                            isIssuerSerialIncluded ? new IssuerSerial(new GeneralNames(new GeneralName(assocCert.getIssuer())), serial)
                                                                   : null);
            ...
            else
            {
                ...
                this.signerInfoGen = new SignerInfoGenerator(signerInfoGen, new CMSAttributeTableGenerator()
                {
                        ...
                        throws CMSAttributeTableGenerationException
                    {
                        ...
                        {
                            return table.add(PKCSObjectIdentifiers.id_aa_signingCertificateV2, new SigningCertificateV2(new ESSCertIDv2[] { essCertid }));
                        }
...
*/
        ...
        throws TSPException
    {
        ...
        Extensions respExtensions = null;
		if (request.hasExtensions())
		{
			List oids = request.getExtensionOIDs();
			Extension[] extensions = new Extension[oids.size()];
			for (int i = 0; i < oids.size(); i++) {
				extensions[i] = request.getExtension(ASN1ObjectIdentifier.getInstance(oids.get(i)));
			}
			respExtensions = new Extensions(extensions);
		}
        ...
        if (resolution == R_SECONDS)
        {
            if (locale != null)
            {
                ...
                timeStampTime = new ASN1GeneralizedTime(dateF.format(genTime));
            }
            else
            {
