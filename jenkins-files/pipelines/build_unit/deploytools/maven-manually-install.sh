...
mvn -Dmaven.repo.local=${MAVEN_REPO_LOCAL} install:install-file -DgroupId=org.signserver.deploytools -DartifactId=DeployTools -Dversion=${DPL_VERSION} -Dpackaging=pom -Dfile=DeployTools-${DPL_VERSION}-pom.xml $@
...
for jar in ./DeployTools-Common-${DPL_VERSION}.jar ./DeployTools-Maven-${DPL_VERSION}.jar; do
